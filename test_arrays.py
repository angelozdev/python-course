...
def test_remove():
    ...
    a.remove(2) # .remove() removes the first occurrence of the element and modifies the list in place
    assert a == [1, 3, 4, 5]

def test_pop():
    a: list[int] = [1, 2, 3]
    assert a.pop() == 3 # .pop() returns the last element and modifies the list in place
    assert a == [1, 2]
